...
import type { GuardianSummary } from '@/shared/types/stock_agents'
...
export const getGuardianSummary = (signal?:AbortSignal) => quantRequest<GuardianSummary>(`${base}/guardian/overview?compact=true`, { signal })
...
export const getAgentHistory = (id: string, kind: AgentHistoryKind, offset = 0, start?: string, end?: string, signal?: AbortSignal, limit = 20) => quantRequest<AgentPage>(`${path(id)}/history${query({ kind, limit, offset, start, end })}`, { signal })
...
export const runStockAgent = (id: string, phase: AgentPhase, requestId: string, researchDate?: string) => quantRequest<{ status: string; run_id: string }>(`${path(id)}/run`, { method: 'POST', body: JSON.stringify({ phase, request_id: requestId, research_date: researchDate || undefined }) })
