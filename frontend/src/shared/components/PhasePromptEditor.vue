...
<template>
  <div class="phase-prompt-editor space-y-3">
    ...
    <p class="text-xs text-muted-foreground leading-relaxed" aria-live="polite">
      ...
      <template v-else-if="stage === 'intraday'">用于盘中、竞价和尾盘，与共用基调一起生效。{{ separateWeekly ? '周复盘使用独立配置；共用偏好请写入共用基调。' : '盘前或盘后未配置时回退到这一份。' }}</template>
      <template v-else>{{ stage === 'premarket' ? '用于盘前计划。' : separateWeekly ? '专用于日复盘，解释本日判断与下一交易日变化。' : '用于盘后研究。' }}{{ text.trim() ? '与共用基调一起生效，不叠加盘中提示词。' : '未独立配置，当前使用共用基调与盘中提示词。' }}</template>
    </p>
    <Textarea :id="inputId" v-model="text" :aria-label="label" :rows="12" :maxlength="stage === 'weekly' ? undefined : maxLength"
      class="h-64 min-h-40 max-h-[45vh] resize-y leading-relaxed [field-sizing:fixed]" :placeholder="stage === 'weekly' ? props.defaultWeeklyPrompt || '留空使用内置整周总结；不继承日复盘或盘中提示词' : stage === 'common' ? '各阶段共用的身份与偏好，无固定偏好可留空' : stage === 'intraday' ? '输入盘中任务与判断方法' : '留空时自动使用盘中提示词'" />
    <div class="flex justify-end">
      ...
      <Button v-else-if="(stage === 'premarket' || stage === 'review') && text" type="button" variant="ghost" size="sm" @click="text = ''">改用盘中提示词</Button>
    </div>
  </div>
</template>
