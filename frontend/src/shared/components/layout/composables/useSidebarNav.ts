 ...
 *
 * 弹层开关（themeOpen）也归这里：底部「主题」是**动作项**而非路由项，
 * 它的分流就在 onFootClick 里，开关跟着分流走才不会两地对不上。
 */
...
export function useSidebarNav() {
  ...
  const footItems = computed<FootItem[]>(() => {
    const items: FootItem[] = [
      ...
      { id: 'ops', label: '设置', icon: Settings, path: '/ops' },
    ]
    if (userStore.isAdmin) {
      items.push({ id: 'admin', label: '管理后台', icon: ShieldCheck, path: '/admin' })
    }
    return items
  })

  /** 只有路由项会亮；动作项（主题）永远不进选中态。 */
