...
export function createCegaoSelectActions(deps: {
  ...
  async function ensureSavedForScreen(): Promise<string | null> {
    ...
    try {
      await ElMessageBox.confirm('选股需要先保存当前公式，是否现在保存？', '保存后选股', {
        type: 'info',
        confirmButtonText: '保存并选股',
        cancelButtonText: '取消',
      })
    } catch {
      return null
  ...
  async function runSelect(payload: {
    ...
    try {
      if (payload.mode === 'single') {
        ...
        if (!formalCount && !watchCount) ElMessage.info('这个范围没有选出股票')
        else ElMessage.success(`正式 ${formalCount} 只，低吸观察 ${watchCount} 只`)
        return
      }
      ...
      if (outcome === 'busy') {
        ElMessage.warning(deps.screenRun.lastError || '已有选股任务在跑')
        return
      }
      if (outcome === 'error') {
        ElMessage.error(deps.screenRun.lastError || '选股启动失败')
        return
      }
      ElMessage.success('区间选股已开始')
