...
展示约定：三种视图共用真实来源计数与工具条。来源卡片使用可缩至容器宽度的栅格；用途卡和接口筛选使用主题表面与边界令牌；接口试跑、批量探测和详情抽屉限制在视口内滚动。耗时使用等宽读数，慢速与失败用告警色并保留文字，减少动态效果的系统偏好会关闭耗时条动画。
...
- `LatencyMeter`：耗时读数条（正常用信息色，慢速和失败用告警色）
