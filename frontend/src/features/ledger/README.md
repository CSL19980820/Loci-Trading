...
候选详情在窄屏切成单列，证据数据可展开查看，保留原文及删除、看档案动作。档案通过 `style scoped src="./ArchiveView.css"` 引用样式，内嵌行情面板使用局部 `:deep()`；同批列表展示序号、名称、涨跌，窄屏抽屉保留视口边距。切票按钮配有图标与键盘说明。
...
- **全屏档案**：`/archive/:code` 通过 App 的 `PageHost` 呈现，源页经 KeepAlive 保活；叠放由当前页面层与源码弹层组件协调。点「返回」/「回本批」或 `Esc` 关层。层级和遮挡效果以浏览器验证为准。
