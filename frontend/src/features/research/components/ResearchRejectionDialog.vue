...
async function reject(): Promise<void> {
  ...
  try {
    ...
    ElMessage.success(result.reused ? '相同人工否决已确认' : '人工否决已记录')
  ...
  >
    ...
    <div class="mt-2 flex flex-col gap-2">
      <UiField label="否决人" required :error="error && !form.reviewer.trim() ? '请填写否决人' : ''">
        <el-input v-model="form.reviewer" aria-label="否决人" maxlength="128" show-word-limit />
      </UiField>
      <UiField label="否决理由" required :error="error && !form.reason.trim() ? '请填写否决理由' : ''">
        <el-input v-model="form.reason" aria-label="否决理由" type="textarea" :rows="3" maxlength="2000" show-word-limit />
      </UiField>
    </div>
    <template #footer>
      <el-button :disabled="submitting" @click="close">取消</el-button>
      <el-button type="danger" :icon="CloseBold" :disabled="!canReject" :loading="submitting" @click="reject">
        记录人工否决
      </el-button>
    </template>
  </el-dialog>
