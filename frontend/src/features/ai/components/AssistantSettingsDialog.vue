...
async function load(): Promise<void> {
  ...
  try {
    ...
    ElMessage.error(toErrorMessage(caught, '加载助手设置失败'))
  } finally {
    loading.value = false
  }
}
...
async function saveProfile(): Promise<void> {
  if (userDoc.value.length > MEMORY_QUOTAS.user) {
    ElMessage.error(`用户画像不得超过 ${MEMORY_QUOTAS.user} 字`)
    return
  }
  if (workDoc.value.length > MEMORY_QUOTAS.memory) {
    ElMessage.error(`工作记忆不得超过 ${MEMORY_QUOTAS.memory} 字`)
    return
  }
  saving.value = true
  try {
...
async function restoreDefaults(): Promise<void> {
  try {
    await ElMessageBox.confirm(
      '将恢复产品默认的回答偏好、规则与工作记忆文档；「关于你」会保留。用户画像文档也会恢复为模板。',
      '恢复产品默认',
