...
- `ReviewCenterView.vue`：绩效中心（侧栏「绩效」）；**无页头**（PageHeader 已下线），页面身份交给侧栏；`PageTabs` 分区，刷新 / 精选短线 T+1·T+3·T+5 行内胜率 / 口径 ⓘ 全在 tabs `trailing`，预案条数走 Tab 徽标；各分区 Colada 按当前 tab `enabled`，不预拉其它分区；候选验证表每页渲染 40 条并显示总数；候选 / 预案 **空态走 `BasicTable` `#empty`**（列头始终露出，动作放表内）
...
- `ReviewsView.vue`：复盘样本列表（`/reviews/records`）；无页头，「补记一笔」与口径 ⓘ 并进 `BasicTable` 自带工具行（`#toolbarButtons`），总数看分页；类型 tag 三色 plain 区分候选/预案/成交（成交仅历史存量，新写入只允许候选/预案）；**侧栏暂不挂入口**（「手记」已撤），深链/胜率「去补复盘」仍可用；后续 AI 复盘可挂靠绩效 Tab 或独立入口；空态同样挂在表内 `#empty`
- `InsightsView.vue`：管家式数据体检（hero 无标题栏，状态字并进标题行；左大体检分环 + 右主 CTA；一键扫描=仓内+本地扩展，深度扫描=另探数据源连通；一键修复 · 待检分组清单 · 自动复检）+「信号重叠」Tab（Sheet 无标题：天数、刷新、口径 ⓘ 全在分区行；战法 A/B 列落中文短名）。idle 印章环为虚线「待检」，状态文案中性灰、仅待处理用告警色。逻辑在 `composables/useHealthCheckup.ts`；后端目录见 `sentinel` + `sentinel_extended`。重叠表始终挂着，空数据走 `empty-text` / `empty-reason`。
...
- `components/HealthCheckList.vue`：idle 待检按 `group` 分组成响应式检查目录（分组区域 + 紧凑 check-row）；结果按阻断→提示分组；一键修复/勾选只认 `autoFixable`（人工项走「去处理」）。
- `WinRateView.vue`：胜率，**两层**。顶栏 `PageToolbar` 给全局读数（综合胜率/样本数/T+5 均收益，由已加载行汇总、不另发请求），口径全文进 ⓘ；`PageTabs` = 「综合对比 + 每个战法」（badge 是 T+5 样本数），分周期粒度选择器放 tabs `trailing`，两层共用。
  - **综合层**（默认）：`components/WinRateCompareTable.vue` 使用同一张表横向比较战法，展示胜率、盈利数/样本数、收益与可比持有期。整行可进入详情，行末样本按钮可通过键盘操作；不另画一组重复表格读数的战法卡片。下方为 `WinRatePeriodTable.vue` 的 `matrix` 形态（周期 × 战法），胜率与收益使用独立文本，热度仅由真实胜率生成。
...
- `ReviewCenterView.vue`：候选验证列含 T+1/3/5/10/20/60；「精选短线兑现」标题与胜率卡片已删（压成 tabs 尾部行内读数），「当初否决、事后大涨」保留在可内滚的紧凑区域，最高三行，避免条目增多挤走表格
...
计算口径 / 结算进分母 / 盈利样本 / 窗口观察中保留在 `.wr-summary__facts` 注脚行；周期区不再重复展示从同一张表推导的四格 KPI。比较表、周期表和样本表不叠加与数字重复的胜率进度条。

**改这一屏之前先问一遍**：要加的这条线/色块/图标，能不能被「它旁边的数字」替代？能的话就不要加。层次交给排版（字号、字重、颜色、留白），不交给边框与色条。

验收应覆盖宽屏与窄屏、长中文名、观察中样本、负均收益与正胜率并存、空样本、加载失败，以及明暗外观。生产界面验收由主代理统一执行，单元测试和类型检查不替代浏览器验收。

### 内层滚动与真实读数

`WinRateView.css` 分配主表/证据与周期区高度；单战法证据在 `WinRateStrategyPanel.css` 约束的内层滚动，窄屏将卡片和表格依次排列。持有期、样本筛选、周期清除与周期柱形都有可聚焦按钮和选中状态，行内按钮阻止冒泡，避免一次点击触发两次筛选。

周期柱形使用真实均收益映射零轴两侧的半区，不设假最小柱高；零收益高度为零，缺失收益显示「—」。最佳/最差样本的颜色依据实际收益正负，不把「最佳」直接视为盈利，也不把最终收益标为回撤。

`InsightsView.css` 让体检内容独立滚动、信号重叠表吃满剩余高度。体检环不在视图中附加虚构的 8%/12% 起步值，而是使用状态机给出的进度；扫描取消只保留一个入口。`HealthCheckList.css` 负责分组与窄屏排布；进度条暴露 progressbar 数值，修复勾选提供检查项名称，已通过分组提供展开状态。所有抽出的 CSS 都通过 `<style scoped src="..." />` 引入。
