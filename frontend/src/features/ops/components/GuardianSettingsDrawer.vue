...
const props = defineProps<{ config: GuardianConfig; defaultPrompt: string; providers: LlmProvider[]; providersLoading?: boolean; busy: boolean; error: string; startOnSave: boolean }>()
...
<template>
  <el-drawer v-model="open" class="guardian-settings-drawer" title="交易员设置" size="min(560px, 100vw)" :before-close="close" :close-on-click-modal="false">
    ...
    <el-form label-position="top" @submit.prevent>
      ...
      <el-form-item label="模型供应商"><el-select v-model="draft.provider" :loading="providersLoading" :disabled="providersLoading" placeholder="选择已配置的供应商" @change="draft.model = ''"><el-option v-for="p in providers" :key="p.id" :label="p.name" :value="p.name" /></el-select></el-form-item>
      <el-form-item label="守护模型"><el-select v-model="draft.model" :loading="providersLoading" :disabled="providersLoading" filterable placeholder="选择模型"><el-option v-for="m in models" :key="m" :label="m" :value="m" /></el-select></el-form-item>
    ...
    <template #footer><div class="settings-footer"><span>{{ dirty ? '有未保存的修改' : '配置已同步' }}</span><el-button :disabled="busy" @click="open = false">取消</el-button><el-button type="primary" :loading="busy" :disabled="providersLoading" @click="emit('save', draft)">保存配置</el-button></div></template>
  </el-drawer>
</template>
<style scoped>
