...
vi.mock('@/shared/api/guardian', () => ({ getGuardianReports: vi.fn(), getGuardianReview: vi.fn(), runGuardianReview: vi.fn() }))
...
it('loads a report, displays full account review and does not refetch unchanged polls', async () => {
  ...
  vi.mocked(getGuardianReports).mockResolvedValue({ items: reports, total: 1 })
  const wrapper = mount(GuardianReviewPanel, { props: { enabled: true } })
  await flushPromises()
...
it('queues today review and exposes a rejected premarket request', async () => {
  ...
  vi.mocked(getGuardianReports).mockResolvedValue({ items: [], total: 0 })
  const wrapper = mount(GuardianReviewPanel, { props: { enabled: true } })
