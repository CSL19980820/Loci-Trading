...
export type GuardianRange = Pick<GuardianHistoryQuery, 'start' | 'end' | 'limit'> & { followToday?: boolean }

export function useGuardianHistory<T>(fetchPage: (query: GuardianHistoryQuery, signal?: AbortSignal) => Promise<GuardianPage<T>>, poll = true) {
  const today = guardianToday()
  const range = ref<GuardianRange>({ start: today, end: today, limit: 20, followToday: true })
  const page = ref(1)
  ...
  let disposed = false
  const query = computed<GuardianHistoryQuery>(() => ({ start: range.value.start, end: range.value.end, limit: range.value.limit, offset: (page.value - 1) * range.value.limit }))
