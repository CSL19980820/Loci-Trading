...
from src.ops.skills import DEFAULT_SKILL_ROOT
from src.ops.store import DEFAULT_DB, JOB_KINDS
...
def cmd_skill_install(args: argparse.Namespace) -> int:
    ...
    with _store(args) as store:
        store.upsert_skill(package.to_record())
...
def cmd_skill_remove(args: argparse.Namespace) -> int:
    with _store(args) as store:
        removed_db = store.delete_skill(args.slug)
...
def cmd_skill_toggle(args: argparse.Namespace) -> int:
    with _store(args) as store:
        if store.get_skill(args.slug) is None:
            print(f"没有找到技能：{args.slug}", file=sys.stderr)
            return 1
        store.set_skill_enabled(args.slug, args.enabled == "on")
...
def cmd_genkey(args: argparse.Namespace) -> int:
    from src.ai.crypto import MASTER_KEY_ENV, generate_master_key

    print(f"{MASTER_KEY_ENV}={generate_master_key()}")
    print()
    print("把这一行写进服务器的 /srv/qianlong-palace/.env，并在", file=sys.stderr)
    print("deploy/docker-compose.yml 的 environment 块加一行同名变量——", file=sys.stderr)
    print("compose 用的是显式白名单，只加进 .env 容器是读不到的。", file=sys.stderr)
    return 0
...
def cmd_job_add(args: argparse.Namespace) -> int:
    from src.ops.scheduler import validate_cron
