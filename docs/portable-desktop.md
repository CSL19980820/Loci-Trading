...
每次启动会清理 `data/webview` 下的 HTTP/代码缓存（保留 Cookie / LocalStorage），并给 `index.html` 与路由壳加 `Cache-Control: no-store`，避免增量部署后仍打开旧 SPA。需要保留磁盘缓存排障时设 `LOCI_SKIP_WEBVIEW_CACHE_PURGE=1`。
