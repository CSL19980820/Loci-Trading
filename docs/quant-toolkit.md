...
| | 通达信 | 早先的做法 | 现在 |
|---|---|---|---|
| 数据 | 本地 `.day` 二进制 | 每次实时打 akshare | 本地 `market.db` |
| 计算 | C 引擎，**逐票循环** | pandas 逐票 `apply` | **面板向量化，一次算完全市场** |
| 全市场选股 | ~10 秒 | 小时级 | **实测外推 4.4 秒** |
...
**实测吞吐**：0.5–0.76 秒/票（新浪源一次返回全历史）。全市场 5400 只，
串行约 43 分钟、4 并发约 11 分钟。400 只 = 271 万行 = 401 MB，
外推全市场约 5.4 GB。
...
对 400 只深市主板、2025-01 至今跑潜龙出海竞价版：

| 配置 | 笔数 | 胜率 | 净收益均值 | MFE均值 | MAE均值 |
|---|---|---|---|---|---|
| 持有 1 日 | 27 | 44.4% | −0.84% | +2.56% | −3.52% |
| 持有 3 日 | 27 | 37.0% | −1.80% | +3.30% | −4.09% |
| 持有 5 日 | 27 | 40.7% | −1.28% | +5.30% | −4.81% |
| 持有 3 日 + 止盈 5% | 27 | **48.1%** | −1.08% | +3.20% | −4.09% |

**MFE 均值 +3.3% 而净收益 −1.8%：浮盈全部回吐了。** 持有越久越差，
说明信号只有当日脉冲；止盈 +5% 把胜率从 37% 拉到 48%，退出纪律有效。

样本仅 27 笔、单一板块、一年区间，**不足以下结论**。少于 30 笔时
`compute_metrics` 会自动附上警告——不加这句，人就会把 7 笔交易的均值
当成结论。
