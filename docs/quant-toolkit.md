...
| | 通达信 | 早先的做法 | 现在 |
|---|---|---|---|
| 数据 | 本地 `.day` 二进制 | 每次实时打 akshare | 本地 `market.db`（5547 只 / 1697 万行 / **4.17 GB**，已回收 1.57 GB） |
...
**实测（全市场已完成回填，2026-08-25 `dbstat`）**：回收前 5,740.2 MB，其中溯源审计表与
索引占 2,569.0 MB（44.8%）。`python -m cli.market reclaim` 删掉无消费者的
`idx_quotes_receipt` 并 VACUUM 后 → **4,172.3 MB（释放 1,567.9 MB）**，`quick_check`
通过、16,966,403 行一行不少。当前口径：5,547 只 / 16,966,403 行 /
**5,740.2 MB**（`quotes_daily` 表体 2,723.0 MB = 160.5 B/行；溯源审计表与索引
2,569.0 MB = **44.8%**）；日期范围 1990-12-19 ~ 2026-08-25。日线主源已换成通达信
本地二进制（见 [ADR-013](adr/ADR-013-tdx-primary-daily-source-and-batch-sync.md)），
`source` 分布 tdx 96.5% / tencent 2.9% / 其余 0.6%。

> 体量的瘦身方案（同一份数据 Parquet+Zstd 只要 395.6 MB，10.6×）见
> [`docs/research/2026-08-mainstream-quant-benchmark.md`](research/2026-08-mainstream-quant-benchmark.md) §6。
...
- **样本与结论**：全市场已回填（5,547 只 / 1,697 万行），但**存活偏差本地不可测**——
  `instruments.delist_date` 填充数为 **0**、`status` 只有 1 条 `delisted`，退市股行情
  未同步。在它修好之前，任何组合层收益数字都只是参考。
- **有效样本量 ≠ 笔数**：同日信号共享市场因子，实测 `ICC = 0.2822`。潜龙原版 24,878 笔
  的有效样本只有 **2,090**（t = 2.13，**不显著**），而分手快乐 495 笔的有效样本 466
  （t = 3.14，显著）。**日均信号数是分母**，别拿笔数当置信度。
...
- **akshare 上游不稳**：底层是爬公开网页接口。近 12 个月发了 **273 个版本**（1.32 天/版），
  **76 个不同 A 股接口被动过**，官方改名 0 次、文档化删除 0 次——**删接口不写 changelog**
  （`stock_hot_rank_wc` 被「修复」6 次后直接消失）。更隐蔽的是**值语义变更**：1.18.73
  «修复 `stock_zh_a_hist_tx` 的成交量/换手率/成交额字段语义» = 列名不动、数值口径全变，
  `source_contract` 100% 抓不到。8 种破坏模式现有防线只覆盖 1.5 种，详见
  [`_scratch_2026-08-qb-akshare-surface.md`](research/_scratch_2026-08-qb-akshare-surface.md) §7。
- **涨停池 6 个接口只承诺「近期」**：越界时上游 `data` 为 `None`，akshare **静默返回空
  DataFrame**——回测会读成「那天没有涨停股」。用它做历史必须显式区分「空」与「取不到」。
- **`ak.stock_info_a_code_name()`**：历史上因 py_mini_racer 执行 JS 在多线程下原生崩溃而
  被弃用；**1.18.56 源码已改为拼接三所列表 + `@lru_cache`，不再需要 JS 引擎**。本仓仍走
  交易所各自的列表接口（更快且源级独立），无需回切。仍依赖 py_mini_racer 的是
  `stock_cyq_em`（筹码，本地跑 JS 重算且固定 90 行）与 `tool_trade_date_hist_sina`。
