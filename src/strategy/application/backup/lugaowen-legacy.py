"""卢高文涨停战法：通达信原式的向量化信号实现（归档，不随 src.strategy 导入）。

产品目录曾默认注册：三外有三、海底捞月、筹码峰突破（均已下线）。
天衣无缝 / 倒拔杨柳 / 分手快乐仍保留类实现，但不 register（避免目录噪音）。

原式把"能量化"和"不能量化"分得很清楚，这里严格照它的边界来：
...
公式都以同一段涨停识别开头（见 src/formula/board.py）。涨跌停幅度
...
class SanwaiYousanV2(SanwaiYousan):
    """三外有三选股优化版，保留原版 ``lugw-sanwai`` 作为对照。"""

    slug = "lugw-sanwai-v2"
    name = "三外有三（优化版）"
    description = "三外有三 V2：回到第三板价附近时限制追高不超过 4%"

    def default_params(self) -> dict[str, Any]:
        return {**super().default_params(), "upper_ratio": 1.04}
...
class FenshouKuaile(_LugaowenBase):
    ...
    def compute(self, panels, params=None) -> SignalResult:
        ...
        names = panels.get("__instrument_names__")
        ratios = limit_ratio_panel(close, names if isinstance(names, dict) else None).clip(upper=0.20)
...
register(SanwaiYousanV2())
# 天衣无缝 / 倒拔杨柳 / 分手快乐：保留实现供单测直调，不进产品目录
