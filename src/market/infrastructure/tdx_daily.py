...
#: 公开行情服务器。数量给够，单台被限流/下线时还能换；探测会按 RTT 排序。
#: 公开行情服务器池。名单取自 tdx 官方主站清单（与 mootdx 内置一致）。
#:
#: **池子要大**：单台常年可用率不高，实测某次只剩 2 台活着；池子小 + 排序
#: 结果落盘缓存，就会出现「缓存钉住的两台同时挂掉、接下来 6 小时整条源静默
#: 全废」。``_connection`` 因此在全池失败时会作废缓存重探一次。
