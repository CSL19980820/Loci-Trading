...
(() => new Promise((resolve) => {
  const api = window.__lociBootSplash;
  if (!api || typeof api.complete !== "function") {
    resolve("missing");
    return;
  }
  api.complete(() => resolve("completed"));
}))()
""".strip()

_CSS = """
html,body{margin:0;height:100%;background:#eef2f6;color:#142033;
font-family:"IBM Plex Sans","Noto Sans SC","Segoe UI","Microsoft YaHei UI",sans-serif}
#boot-splash,.loci-boot-root{position:fixed;inset:0;z-index:9999;overflow:hidden;
background:#eef2f6;transition:opacity .32s ease}
...
.loci-boot-mark{position:relative;width:64px;height:64px;border-radius:8px;
background:#c41e3a;color:#fff;display:grid;place-items:center;
font-family:Georgia,"Noto Serif SC","Songti SC",serif;font-weight:700;font-size:22px;
outline:1px solid rgba(255,255,255,.28);outline-offset:-7px}
.loci-boot-name{font-family:Georgia,"Noto Serif SC","Songti SC",serif;
font-size:26px;font-weight:700;color:#142033;line-height:1}
...
.loci-boot-kicker{font-family:Consolas,"IBM Plex Mono",monospace;font-size:10px;
letter-spacing:.2em;color:#5b6b7c;margin-bottom:4px}
.loci-boot-tip{font-family:"Noto Serif SC",Georgia,serif;font-size:15px;font-weight:600;
color:#142033;letter-spacing:.05em;line-height:1.35}
...
.phase-exit,.phase-error{background:
  radial-gradient(ellipse 50% 36% at 50% 42%,rgba(196,30,58,.04),transparent 70%),#eef2f6}
...
background:linear-gradient(90deg,transparent,#c41e3a 18%,#c41e3a 82%,transparent);
transform-origin:center;animation:loci-close-rail .65s cubic-bezier(.22,1,.36,1) .12s both}
.phase-error .loci-boot-mark{background:#5b6b7c}
.phase-error .loci-boot-close-rail{
background:linear-gradient(90deg,transparent,#8a96a5 18%,#8a96a5 82%,transparent)}
...
def _body_enter(*, tip: str = "晨光落印，账本启封") -> str:
...
<div class="loci-boot-copy loci-tip-blink">
  <div class="loci-boot-kicker"><span class="loci-boot-phase">开账进行中</span></div>
