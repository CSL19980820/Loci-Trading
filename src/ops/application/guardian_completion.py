...
def complete_decision(provider: Any, store: Any, *, system: str, payload: dict, schemas: list,
                      execute: Any, archive: Any, checkpoint: Any, deadline: float, config: dict) -> tuple[GuardianDecision, dict]:
    ...
    arguments = dict(system=system, user_prompt=json.dumps(payload, ensure_ascii=False, default=str),
                     ...
                     on_event=checkpoint, check_cancelled=checkpoint, deadline=deadline,
    ...
    try:
        for attempt in range(2):
            ...
            try:
                ...
                decision = parse_decision(result.text, require_execution_terms=True)
                ...
                messages.append(ChatMessage(role="user", content=f"上次完整性/JSON契约校验失败：{diagnostic['error']}。基于已取得事实输出完整合法决策，不拼接残片，不调用工具。买卖必须明确execution；无法确认条件则hold/watch。"))
                arguments = dict(system=system, messages=messages, max_rounds=1, max_calls_per_round=0,
                                 max_tokens=provider.max_output_tokens or 328000, temperature=0, thinking=config.get("thinking", ""),
                                 allow_hitl=False, on_event=checkpoint, check_cancelled=checkpoint, deadline=deadline)
    except BaseException as exc:
