...
from src.ops.application.guardian_config import (
    ...
    DEFAULT_REVIEW_PROMPT, POSITION_RULES,
)
from src.ops.application.trading_prompts import trading_prompt
from src.ops.application.guardian_weekly_prompt import DEFAULT_WEEKLY_PROMPT, WEEKLY_REVIEW_TASK
...
相同主题修订沿用稳定英文id。每条用hypothesis表达精炼结论和适用边界，validation_plan说明如何验证或什么会推翻。保持原验证状态，不把单例、事后解释或多日报重复观察升级为确定规律；没有实际计算或工具回执，不声称验证已完成。
...
你是同一自主交易员的计划与复盘入口，不是只负责写摘要的角色。可以研究、比较、计算、提出新解释、承认未知和改变判断。阶段任务不同，不代表能力不同。
...
def review_system(cfg: dict, period: str, schema: dict, *, stage: str = "report") -> str:
    ...
    if stage == 'planning':
        return "\n".join((str(cfg.get('common_prompt') or ''),
            '你是自主交易员，现在独立制定下一交易日的研究、观察与条件计划。根据当前账户和新研究自主选择对象、方法、时间与资金安排。量化候选只是来源。可以决定买、卖、继续观察、移除或不采取行动；不要求给每个候选安排动作。',
            ...
            REPORT_WRITING_RULES, json.dumps(schema, ensure_ascii=False)))
    task = ('本次只评价已发生的决策、结果与研究发现，经验可按证据修订。stock_reviews评估已发生的行动和当前证据。下一日买卖计划与观察调整由随后独立的规划阶段生成，本次JSON不输出plans、next_steps、watchlist_updates。'
            if stage == 'retrospective' else REPORT_PHASE_TASKS.get(period, REPORT_PHASE_TASKS['daily']))
    return "\n".join((trading_prompt(effective, period, default),
        "【当前阶段任务与输出契约】", task,
        AUTONOMY_RULES, POSITION_RULES, REPORT_CONTEXT, REPORT_WRITING_RULES, EXPERIENCE_RULES,
        REVIEW_FIELD_WRITING, json.dumps(schema, ensure_ascii=False)))
