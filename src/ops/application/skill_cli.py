...
模型只能调用 frontmatter 声明过的 command；可执行路径必须落在 skill 根内
（或解析为解释器 ``python`` / ``py``，脚本参数仍须在 skill 根内）。
"""
...
def build_command(
    ...
    for index, raw in enumerate(command):
        ...
        if not token:
            continue
        if index == 0 and token.lower() in _INTERPRETER_NAMES:
            argv.append(_resolve_python())
            continue
