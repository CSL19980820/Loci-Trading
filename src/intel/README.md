...
- **brief 三文件分工**（原 602 行超 600 行硬规则，2026-09 按职责拆开；对外仍只从 `src.intel.application.brief` 导入 `build_intel_brief` / `empty_intel_brief` / `BRIEF_TOOLS` / `PROMOTION_RATE_BASIS`）：
  - `brief.py` — 编排入口兼门面：取哪些工具的快照、拼成哪几段、缺数据时长什么样。**不碰单个字段名**
  - `brief_payload.py` — 载荷解析原语（`_number` / `_percent` / `_walk` / `_find_mapping` / `_list_under` 等）：只认 dict 和数字，不认业务。动一下就是十段一起变
  - `brief_sections.py` — 逐段投影，每个函数与悟道**某一张表的字段名**一一绑定；上游改字段只波及一个函数。段落只增不减（2026-08-31 一次进了六段），所以不与编排入口同住
