...
- **当前配额账（默认托管配置，盘中 24 轮/日）**：
  ...
  structured 合计 **3675 → 1755**（预算 3000，已压在预算内）；skill 侧从约 0 涨到约 300~600（预算 2000）。tape 那一行是量级估计（缓存命中率随行情波动），structured 三档是 `estimate_daily_calls` 直接算出来的上限。
...
- **盘面只读摘要**：`build_intel_brief(store, trade_date=…)` / `GET /api/intel/brief` 从缓存投影情绪/题材/梯队；**不调 MCP**、不发明数字；未跑 `intel_fetch` 时 `available=false`。列表读缓存用 `list_latest_snapshots`（同工具多参取最新 `fetched_at`）。情绪字段兼容悟道 `sealedLimitUp` / `brokenBoardRate` / `promotionRates.firstToSecond`；晋级率与炸板率统一成 0–100 百分数（`_percent`）；题材附带 `pct_chg` / `main_net_amount_text`，避免把开盘啦内部 strength 当人话展示。
