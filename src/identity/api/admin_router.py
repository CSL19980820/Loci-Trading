...
def build_admin_router(*, auth_dependency: Any, identity_db: str | None = None) -> APIRouter:
    ...
    def llm_usage(
        ...
        from src.ai.application.usage_reporting import platform_model_usage
        from src.shared.paths import data_dir
        ...
        if last < first or (last - first).days > 366:
            raise HTTPException(status_code=422, detail="日期范围须为 0 至 366 天")
        return platform_model_usage(data_dir(), start=first, end=last)
    ...
    def audit(
        ...
        """账号、权限及配置变更审计；认证事件在独立登录日志中展示。"""
        _admin(context)
        with _store() as store:
            return {
                "items": store.list_audit(
                    actor_id=actor_id,
                    exclude_actions=platform.LOGIN_ACTIONS,
                    keyword=keyword,
                    action=action,
                    outcome=outcome,
                    limit=limit,
    ...
    def logins(
        ...
        with _store() as store:
            return {
                ...
                "total": store.count_audit(
                    keyword=keyword, action=action, outcome=outcome, actions=platform.LOGIN_ACTIONS
                ),
            }
