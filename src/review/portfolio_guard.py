...
class GuardResult:
    allowed: bool
    violations: list[str]
    allowed_codes: list[str]  # codes that pass (violations dropped)
...
def check_portfolio_limits(
    ...
    2. new_picks 逐一检查：超出总额 / 单战法额度则丢弃，行业超限仅警告。
    """
    if limits is None:
    ...
    # 因为行业超限只警告不丢弃，我们先把所有 new_picks 算进分母
    ...
    for pick in new_picks:
        ...
        if remaining_total <= 0:
            violations.append(f"{code}: 超出总持仓上限 {limits.max_total_positions}")
            continue
