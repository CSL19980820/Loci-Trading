"""把 src/shared/boot_splash.py 的 _CSS 对齐到 frontend/index.html 里那份。
...
MODULE = ROOT / "src" / "shared" / "boot_splash.py"
...
def main() -> None:
    ...
    if not pattern.search(source):
        raise SystemExit("boot_splash.py 里找不到 _CSS 三引号块")

    def _sub(m: re.Match[str]) -> str:
        return f"{m.group(1)}{css}\n{m.group(3)}"

    MODULE.write_text(pattern.sub(_sub, source, count=1), encoding="utf-8")
    print(f"已把 {len(css.splitlines())} 行 CSS 从 index.html 回灌到 boot_splash.py")
