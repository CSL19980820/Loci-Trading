...
from src.ai.client import ChatResponse, ProviderConfig, ToolCall
from src.ai.crypto import generate_master_key
from src.intel.mcp import McpClient, McpError, McpTool, _parse_response
from src.intel.registry import build_client, collect_tools, save_server
...
class RegistryTests(unittest.TestCase):
    ...
    def _save(self, **overrides):
        params = {
            "name": "wudao", "url": "https://example.com/mcp", "token": "lb_secret_1234",
            "verify": False, "master_key": self.key,
        }
        params.update(overrides)
        return save_server(self.store, **params)

    def test_token_is_stored_encrypted_and_never_echoed(self) -> None:
        record = self._save()
        self.assertEqual(record["token_last4"], "****1234")
        self.assertNotIn("encrypted_token", record)
        self.assertTrue(record["has_token"])

        raw = self.store.get_mcp_server("wudao", include_secret=True)
        self.assertIsInstance(raw["encrypted_token"], bytes)
        self.assertNotIn(b"lb_secret_1234", raw["encrypted_token"])

    def test_client_is_rebuilt_with_the_decrypted_token(self) -> None:
        self._save()
        client = build_client(self.store, "wudao", master_key=self.key)
        self.assertEqual(client.token, "lb_secret_1234")

    def test_ciphertext_cannot_be_moved_to_another_server(self) -> None:
        """AAD 绑定 server id：密文搬家就解不开。

        没有这层绑定，攻击者可以把 A 的密文搬到 B，然后拿着 A 的 token 去
        请求 B 声明的 URL——等于把凭据送出去。
        """
        self._save()
        stolen = self.store.get_mcp_server("wudao", include_secret=True)["encrypted_token"]
        self._save(name="evil", url="https://attacker.example/mcp", token="x")
        evil = self.store.get_mcp_server("evil")
        self.store.upsert_mcp_server(
            {"id": evil["id"], "name": "evil", "url": evil["url"],
             "encrypted_token": stolen, "token_last4": "****1234"}
        )
        with self.assertRaises(Exception):
            build_client(self.store, "evil", master_key=self.key)
    ...
    def test_updating_without_a_token_keeps_the_existing_one(self) -> None:
        ...
        record = self.store.get_mcp_server("wudao")
        self.assertEqual(record["url"], "https://example.com/mcp2")
        self.assertEqual(record["token_last4"], "****1234")
    ...
    def test_collect_tools_narrows_to_the_allow_list(self) -> None:
        ...
        self.store.upsert_mcp_server(
            {
                "name": "wudao", "url": "https://example.com/mcp",
                "tools": [
        ...
        tools, routing = collect_tools(self.store, ["wudao"], allow=["kline", "limit_up_ladder"])
    ...
    def test_inactive_servers_are_excluded(self) -> None:
        self.store.upsert_mcp_server(
            {"name": "off", "url": "https://x/mcp", "is_active": False,
             "tools": [{"name": "t", "description": "d"}]}
        )
        tools, _ = collect_tools(self.store)
...
class AgentLoopTests(unittest.TestCase):
    ...
    def test_returns_directly_when_no_tools_are_requested(self) -> None:
        with patch("src.ai.agent.chat", return_value=ChatResponse(text="结论", model="m")):
    ...
    def test_executes_tools_then_continues(self) -> None:
        ...
        with patch("src.ai.agent.chat", side_effect=responses):
            result = run_agent(
    ...
    def test_stops_at_the_round_limit_and_says_so(self) -> None:
        ...
        with patch("src.ai.agent.chat", return_value=looping):
            result = run_agent(
                self.config, system="s", user_prompt="q",
    ...
    def test_caps_tool_calls_per_round(self) -> None:
        ...
        with patch("src.ai.agent.chat", side_effect=[many, done]):
            result = run_agent(
                self.config, system="s", user_prompt="q",
                tool_schemas=[{}], tool_executor=lambda n, a: {"text": "x"},
