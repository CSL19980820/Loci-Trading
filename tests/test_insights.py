...
from src.review.decay import StrategyDecayReport, check_all_decay, check_decay
from src.review.overlap import OverlapReport, compute_overlap
from src.review.portfolio_guard import (
    GuardResult,
    PortfolioLimits,
...
class PortfolioGuardTests(unittest.TestCase):
    ...
    def test_excess_positions_dropped(self) -> None:
    ...
    def test_per_strategy_limit_enforced(self) -> None:
        limits = PortfolioLimits(max_total_positions=20, max_positions_per_strategy=2)
        picks = [
            {"code": "000001", "rule_version": "strat-a"},
            {"code": "000002", "rule_version": "strat-a"},
            {"code": "000003", "rule_version": "strat-a"},  # 超出单战法上限
        ]
        result = check_portfolio_limits(picks, existing_core=[], limits=limits)
        self.assertIn("000001", result.allowed_codes)
        self.assertIn("000002", result.allowed_codes)
        self.assertNotIn("000003", result.allowed_codes)
    ...
    def test_partial_existing_reduces_budget(self) -> None:
        limits = PortfolioLimits(max_total_positions=3, max_positions_per_strategy=5)
        existing = [{"code": "000099", "rule_version": "strat-z"}]
        picks = [
            {"code": "000001", "rule_version": "strat-a"},
            {"code": "000002", "rule_version": "strat-a"},
            {"code": "000003", "rule_version": "strat-a"},  # 超过剩余 2 个槽位
        ]
        result = check_portfolio_limits(picks, existing_core=existing, limits=limits)
        self.assertEqual(len(result.allowed_codes), 2)
