...
def test_cancel_endpoint_marks_running_job_for_cooperative_stop(tmp_path: Path) -> None:
    ops_db = tmp_path / "ops.db"
    with OpsStore(ops_db) as store:
        job_id = store.create_job(name="cancelable", kind="sync")
        job = store.get_job(job_id)
        assert job is not None
        run_id = store.start_run(job)
    ...
    app.include_router(build_jobs_router(write_dependency=lambda: None, ops_db=str(ops_db)))
    with TestClient(app, raise_server_exceptions=False) as client:
        response = client.post(f"/api/jobs/runs/{run_id}/cancel")
