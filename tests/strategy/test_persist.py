...
class ScreenRunStateTests(unittest.TestCase):
    ...
    def test_screen_run_refreshes_spot_when_window_includes_today(self) -> None:
        """窗口含今天时先刷 spot，再跑选股（与 job:screen 对齐）。"""
        from datetime import date

        today = date.today().isoformat()
        spot_calls: list[tuple[list[str], bool]] = []
        ...
        def fake_spot(store, codes, *, instrument_types=None, raise_on_failure=False, **_kw):
            spot_calls.append((list(codes), bool(raise_on_failure)))
            return len(codes)
        ...
        with (
            ...
            patch("src.market.apply_today_spot", side_effect=fake_spot),
        ):
            execute_screen_run(
                {
                    "strategy": "qianlong-tail-v1",
                    "start": today,
                    "end": today,
                    "record_candidates": False,
                    "skip_health_check": True,
        ...
        self.assertEqual(len(spot_calls), 1)
        self.assertEqual(spot_calls[0][0], ["600519", "000001"])
        self.assertTrue(spot_calls[0][1])
        self.assertTrue(any("当日 spot" in line for line in snap["log"]))
        screen_run_update(status="idle", result=None, error="", log=[])
    ...
    def test_screen_run_can_skip_spot_refresh(self) -> None:
        ...
        def boom(*_a, **_k):
            raise AssertionError("refresh_spot=false 不应调用 apply_today_spot")
        ...
        with (
            patch("src.strategy.screen", side_effect=fake_screen),
            patch("src.market.apply_today_spot", side_effect=boom),
        ):
            execute_screen_run(
                {
                    "strategy": "demo",
