...
from src.ai.crypto import CryptoError, decrypt_secret, encrypt_secret, generate_master_key, mask_secret
...
class JobExecutionTests(unittest.TestCase):
    ...
    def test_disabled_skill_cannot_be_run(self) -> None:
        self.store.upsert_skill(
            {"slug": "s1", "name": "S1", "description": "d", "instructions": "do it",
             "install_path": "/tmp/s1"}
        )
        self.store.set_skill_enabled("s1", False)
        job_id = self.store.create_job(
...
class SkillSystemPromptTests(unittest.TestCase):
    def test_iron_rules_precede_and_outrank_skill_instructions(self) -> None:
        ...
        """
        from src.ops.jobs import SKILL_SYSTEM_PREFIX
